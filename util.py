...
class Translator(object):
    ...
    def model(self, ):
        ...
        with tf.variable_scope("Translator", reuse=tf.AUTO_REUSE, initializer=self.init):
            ...
            decoder_output = self.build_decoder(
                encoder_state, inputs_dec, self._is_training)
            logits = self.fully_connected(decoder_output)

            logits = tf.reshape(
                logits, [self._batch_size, self.nr_steps_dec, self._vocab_size])
    ...
    def fully_connected(self, decoder_output):
        W = tf.get_variable("Weights_fc", dtype=tf.float32, shape=[
                            self._hidden_size, self._vocab_size])
        b = tf.get_variable("Bias_fc", dtype=tf.float32,
    ...
    def run_model(self, operation, verbose=False):
        ...
        costs = 0.0
        iters = 0
        for _ in range(self._batch_nr // (self._epoch_size * self._batch_size)):
